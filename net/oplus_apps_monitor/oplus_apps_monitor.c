				      ...
				      int payload_len)
{
	...
	/* send data */
	if (apps_monitor_netlink_pid) {
		ret = netlink_unicast(apps_monitor_netlink_sock, skbuff,
				      apps_monitor_netlink_pid, MSG_DONTWAIT);

	} else {
		printk(KERN_ERR
		       "oplus_apps_monitor: apps_monitor_netlink_send_to_user, can not unicast skbuff, apps_monitor_netlink_pid=0\n");
		kfree_skb(skbuff);
	}

	if (ret < 0) {
		printk(KERN_ERR
		       "oplus_apps_monitor: apps_monitor_netlink_send_to_user, can not unicast skbuff, ret = %d\n",
		       ret);
		return -1;
	}
...
static int apps_monitor_netlink_nlmsg_handle(struct sk_buff *skb,
...
static int apps_monitor_netlink_init(void)
{
	...
	apps_monitor_netlink_sock = netlink_kernel_create(&init_net,
				    NETLINK_OPLUS_APPS_MONITOR, &cfg);
