...
struct oplus_sla_rom_update_info {
	...
	//Add for game latency
	u32 game_latency;     /* game latency threshold */
	//end
};
...
static int oplus_smart_bw_set_params(struct nlmsghdr *nlh)
{
	...
	u32 count = (nlh)->nlmsg_len -
		    NLMSG_HDRLEN; /* this is in fact the payload length which already aligned */
	...
	if (1) {
		printk("oplus_smart_bw_set_params: (nlh)->nlmsg_len = %u, NLMSG_HDRLEN=%d,"
		       ...
		       (nlh)->nlmsg_len, NLMSG_HDRLEN, count,
