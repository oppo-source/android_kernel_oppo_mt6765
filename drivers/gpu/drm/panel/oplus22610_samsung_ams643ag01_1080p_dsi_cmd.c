...
u32 flag_hbm;
u32 flag_hbm_dimming;
u32 flag_dimming_dc;
u32 flag_hbm_dimming_V2;
...
static int panel_doze_enable_start(struct drm_panel *panel, void *dsi, dcs_write_gce cb, void *handle)
{
	...
	pr_err("debug for lcm %s\n", __func__);
	for (i = 0; i < sizeof(lcm_dimming_off)/sizeof(struct LCM_setting_table); i++){
		cb(dsi, handle, lcm_dimming_off[i].para_list, lcm_dimming_off[i].count);
	}
	cmd = 0x28;
	cb(dsi, handle, &cmd, 1);
	//cmd = 0x10;
		...
		void *handle, unsigned int level)
{
	...
	pr_err("debug for lcm %s, level = %d, mapped_level = %d, last_backlight_level = %d\n",
			__func__, level, mapped_level, last_backlight_level);
	if ((flag_hbm_dimming == 0) && (flag_hbm_dimming_V2 == 0) && (level > 1)
		&&(flag_dimming_dc == 0) && (last_backlight_level > 1)) {
		for (i = 0; i < sizeof(lcm_dimming_on)/sizeof(struct LCM_setting_table); i++){
			cb(dsi, handle, lcm_dimming_on[i].para_list, lcm_dimming_on[i].count);
		}
		flag_hbm_dimming = 1;
	} else if (flag_dimming_dc == 1){
		for (i = 0; i < sizeof(lcm_dimming_off)/sizeof(struct LCM_setting_table); i++){
			cb(dsi, handle, lcm_dimming_off[i].para_list, lcm_dimming_off[i].count);
		}
		flag_dimming_dc = 0;
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "oplus22610_samsung_ams643ag01_1080p_dsi_cmd,lcm", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "oplus22610_samsung_ams643ag01_1080p_dsi_cmd,lcm",
		.owner = THIS_MODULE,
