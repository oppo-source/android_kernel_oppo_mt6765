...
static void read_eeprom_xtalk(void)
{
	...
	for (idx = 0; idx <288; idx++) {
		...
		ov32a_xtalk_setting[2*idx + 1] = read_cmos_eeprom_8(eeprom_xtalk_addr, SLAVEADDR);
	}
}
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
    if (enable) {
        ...
        write_cmos_sensor(0x4300, 0x01);
        write_cmos_sensor(0x4302, 0x03);
        write_cmos_sensor(0x4304, 0x05);
        write_cmos_sensor(0x4306, 0x07);
    }
	spin_lock(&imgsensor_drv_lock);
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 2995000;
		break;
#endif
	default:
	break;
