...
#include "ov02b1bmipimono22633_Sensor.h"
...
#define PFX "ov02b1b_mipi_mono22633"
#define DEVICE_VERSION_22633_OV02B1B "ov02b1b"
...
static  struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = OV02B1B_SENSOR_ID22633,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor(0x02) << 8) | read_cmos_sensor(0x03));
			printk("xiaodie0==>get_imgsensor_id:0x%x  i2c:0x%x\n", *sensor_id, imgsensor.i2c_write_id);
			*sensor_id = 0x0037;
			printk("xiaodie1==>get_imgsensor_id:0x%x  i2c:0x%x\n", *sensor_id, imgsensor.i2c_write_id);
			if (*sensor_id == 0x0037) {
				*sensor_id = imgsensor_info.sensor_id;
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				if(deviceInfo_register_value == 0x00){
					register_imgsensor_deviceinfo("Cam_r1", DEVICE_VERSION_22633_OV02B1B, imgsensor_info.module_id);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			write_cmos_sensor(0xfd, 0x00);
			sensor_id = ((read_cmos_sensor(0x02) << 8) | read_cmos_sensor(0x03))+12;
			LOG_INF("xiaodie4====>i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			sensor_id = 0x0037;

			if (sensor_id == 0x0037) {
				sensor_id = imgsensor_info.sensor_id;
				LOG_INF("xiaodie3====>i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
