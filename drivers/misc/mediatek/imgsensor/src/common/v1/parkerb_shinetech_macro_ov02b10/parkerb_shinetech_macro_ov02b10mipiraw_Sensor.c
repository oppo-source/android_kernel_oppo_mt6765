...
#include "parkerb_shinetech_macro_ov02b10mipiraw_Sensor.h"
#define EEPROM_WRITE_ID 0xA4
#define PFX "PARKERB_SHINETECH_MACRO_OV02B10_camera_sensor"
#define LOG_INF(format, args...)    \
	pr_debug(PFX "[%s] " format, __func__, ##args)

/* Camera Hardwareinfo */
//#define DEVICE_VERSION_PARKERB_SHINETECH_MACRO_OV02B10  "parkerb_shinetech_macro_ov02b10"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = PARKERB_SHINETECH_MACRO_OV02B10_SENSOR_ID,
...
static kal_uint16 read_cmos_sensor_shinetech_otp(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
...
static void read_parkerb_shinetech_macro_ov02b10_QRSerialdata(UINT8 *read_parkerb_shinetech_macro_ov02b10_QRSerialdata,
	UINT32 *feature_para_len)
{
	...
	for(i = 0; i < 17; i++){
		read_parkerb_shinetech_macro_ov02b10_QRSerialdata[i] = (kal_int8)read_cmos_sensor_shinetech_otp(start_addr + i);
		checksum = checksum + read_parkerb_shinetech_macro_ov02b10_QRSerialdata[i];
	}
	checksum = checksum % 255;
	if(read_cmos_sensor_shinetech_otp(start_addr+17) == 1 && read_cmos_sensor_shinetech_otp(start_addr+18) == checksum ) {
		*feature_para_len = i;
		LOG_INF("parkerb_shinetech_macro_ov02b10_sn checksum sucess, feature_para_len；%d", *feature_para_len);
	} else {
		*feature_para_len = 0;
		LOG_INF("parkerb_shinetech_macro_ov02b10_sn checksum failed");
	}
}
...
static kal_uint32 return_sensor_id(void)
{
    ...
    return (((read_cmos_sensor(0x02) << 8) | read_cmos_sensor(0x03)) + 5);
}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    if((gain >= 0x40) && (gain <= (15*0x40))) //base gain = 0x40
    {
        ...
        if(iReg<=0x10)
        {
            ...
            write_cmos_sensor(0xfe, 0x02);//fresh
            LOG_INF("PARKERB_SHINETECH_MACRO_OV02B10MIPI_SetGain = 16");
        }
        else if(iReg>= 0xf8)//gpw
        {
            ...
            write_cmos_sensor(0xfe, 0x02);//fresh
            LOG_INF("PARKERB_SHINETECH_MACRO_OV02B10MIPI_SetGain = 160");
        }
        else
        {
            write_cmos_sensor(0xfd, 0x01);
            ...
            write_cmos_sensor(0xfe, 0x02);//fresh
            LOG_INF("PARKERB_SHINETECH_MACRO_OV02B10MIPI_SetGain = %d",iReg);
        }
    }
    else
...
static kal_uint16 read_module_id(void)
{
    ...
    iReadRegI2C(pusendcmd, 2, (u8 *)&get_byte, 1, 0xA4/*EEPROM_READ_ID*/);
    LOG_INF("parkerb_shinetech_macro_ov02b10 the module id is %d\n", get_byte);
    return get_byte;
}
...
//                    hardwareinfo_set_prop(HARDWARE_MONO_CAM_MOUDULE_ID, "Hlt");
					if(deviceInfo_register_value == 0x00){
						register_imgsensor_deviceinfo("Cam_r2", DEVICE_VERSION_OV02B10, module_id);
						deviceInfo_register_value = 0x01;
					}
                    LOG_INF("PARKERB_SHINETECH_MACRO_OV02B10 get_imgsensor_id success: 0x%x\n", *sensor_id);
                    return ERROR_NONE;
                }
            }
    ...
    if (*sensor_id != imgsensor_info.sensor_id) {
        printk("PARKERB_SHINETECH_MACRO_OV02B10 get_imgsensor_id failed: 0x%x\n", *sensor_id);
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {
                printk("PARKERB_SHINETECH_MACRO_OV02B10 open success: 0x%x\n", sensor_id);
                break;
            }
...
{
	pr_info("[parkerb_shinetech_macro_ov02b10] preview mode start\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[parkerb_shinetech_macro_ov02b10] capture mode start\n");
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[parkerb_shinetech_macro_ov02b10] normal_video mode start\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[parkerb_shinetech_macro_ov02b10] hs_video mode start\n");
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[parkerb_shinetech_macro_ov02b10] slim_video mode start\n");
...
static kal_uint32 custom1(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
    MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[parkerb_shinetech_macro_ov02b10] custom1 mode start\n");
		...
		UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_SERIANO_IC:
		read_parkerb_shinetech_macro_ov02b10_QRSerialdata(feature_data_8, feature_para_len);
