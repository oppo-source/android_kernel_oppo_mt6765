...
#include "parkera_qtech_front_imx355mipiraw_Sensor.h"
//#include "imgsensor_common.h"

#define PFX "parkera_qtech_front_IMX355_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = PARKERA_QTECH_FRONT_IMX355_SENSOR_ID_ORIGINAL,
...
static kal_uint16 read_cmos_sensor_qtech_otp(kal_uint16 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
...
static void read_parkera_qtech_front_imx355_QRSerialdata(UINT8 *read_parkera_qtech_front_imx355_QRSerialdata,
	UINT32 *feature_para_len)
{
	...
	for(i = 0; i < 17; i++){
		read_parkera_qtech_front_imx355_QRSerialdata[i] = (kal_int8)read_cmos_sensor_qtech_otp(start_addr + i);
		checksum = checksum + read_parkera_qtech_front_imx355_QRSerialdata[i];
	}
	checksum = checksum % 255;
	if(read_cmos_sensor_qtech_otp(start_addr+17) == 1 && read_cmos_sensor_qtech_otp(start_addr+18) == checksum ) {
		*feature_para_len = i;
		LOG_INF("parkera_qtech_front_imx355_sn checksum sucess, feature_para_len；%d", *feature_para_len);
	} else {
		*feature_para_len = 0;
		LOG_INF("parkera_qtech_front_imx355_sn checksum failed");
	}
}
		...
		do {
			*sensor_id = (((read_cmos_sensor_8(0x0016) << 8)
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
                ...
                // register_imgsensor_deviceinfo("Cam_f", DEVICE_VERSION_IMX355,imgsensor_info.module_id);
				    Eeprom_DataInit(1, PARKERA_QTECH_FRONT_IMX355_SENSOR_ID);
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	read_parkera_qtech_front_imx355_QRSerialdata(feature_data_8, feature_para_len);
