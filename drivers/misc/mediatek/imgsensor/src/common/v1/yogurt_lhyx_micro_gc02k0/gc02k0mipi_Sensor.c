...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = YOGURT_LHYX_MICRO_GC02K0_SENSOR_ID,
	.checksum_value = 0xf7375923,

	.pre = {
		.pclk = 37000000,/*darfur change 0730 24 592M*/
		.linelength = 1079,
		.framelength = 1248,
		.startx = 0,
		.starty = 0,
		...
		.mipi_pixel_rate = 59200000,/*darfur change 0730 24 592M*/
		.max_framerate = 300,
	},
	.cap = {
		.pclk = 37000000,
...
/* 0807 darfur add 24fps 24M592M set_dummy_dual*/
static void set_dummy_dual(void)/*darfur change 0807 592M 24fps*/
{
	kal_uint32  vb = 0xC4;
	kal_uint32  basic_line = 1232;
	/*darfur 0730 24  592M */
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < GC02K0_SENSOR_GAIN_BASE)
		reg_gain = GC02K0_SENSOR_GAIN_BASE;
	else if (reg_gain > GC02K0_SENSOR_GAIN_MAX)
		reg_gain = GC02K0_SENSOR_GAIN_MAX;
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xf8, 0x24);/*darfur 0730 24  592M */
	...
	write_cmos_sensor(0x02, 0x84);/*darfur 0731 0x87*/
	...
	write_cmos_sensor(0x26, 0x07);/*darfur 0730 24  592M */
	write_cmos_sensor(0x29, 0x06);
	write_cmos_sensor(0x2a, 0x08);
	write_cmos_sensor(0x2b, 0x07);/*darfur 0730 24  592M */
	write_cmos_sensor(0xfe, 0x00);
}
...
/* 0807 darfur add dualcamera 24fps*/
static void custom_setting(void)
{
	LOG_INF("E\n");
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x07, 0x00);/* 0807 darfur add dualcamera 24fps*/
	write_cmos_sensor(0x08, 0xC0);/* 0807 darfur add dualcamera 24fps*/

	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xed, 0x90);
}/* 0807 darfur add dualcamera */
...
#ifdef ENABLE_GC02K0_OTP
				if(read_module_id(0x0000) == 0x49){
					*sensor_id = imgsensor_info.sensor_id;
					//hardwareinfo_set_prop(HARDWARE_MONO_CAM_MOUDULE_ID, "Lhyx");
					pr_info("LHYX i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
					return ERROR_NONE;
				}
				else{
					LOG_INF("LHYX Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
...
UINT32 GC02K0_LHYX_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
