...
#include "parkerb_hlt_micro_gc02m1_mipiraw_Sensor.h"
#define ENABLE_GC02M1_OTP 1
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = PARKERB_HLT_MICRO_GC02M1_SENSOR_ID,
...
kal_uint16 addr_data_pair_init_gc02m1_parkerb[] = {
	/*system*/
	0xfc, 0x01,
	0xf4, 0x41,
	0xf5, 0xc0,
...
kal_uint16 addr_data_pair_preview_gc02m1_parkerb[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};

kal_uint16 addr_data_pair_capture_gc02m1_parkerb[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};

kal_uint16 addr_data_pair_normal_video_gc02m1_parkerb[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};

kal_uint16 addr_data_pair_hs_video_gc02m1_parkerb[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};

kal_uint16 addr_data_pair_slim_video_gc02m1_parkerb[] = {
	0xfe, 0x00,
	0x3e, 0x90,
};
...
static void sensor_init(void)
{
	...
	gc02m1_table_write_cmos_sensor(
		addr_data_pair_init_gc02m1_parkerb,
		sizeof(addr_data_pair_init_gc02m1_parkerb) /
		sizeof(kal_uint16));
}
...
static void preview_setting(void)
{
	LOG_INF("E\n");
	gc02m1_table_write_cmos_sensor(
		addr_data_pair_preview_gc02m1_parkerb,
		sizeof(addr_data_pair_preview_gc02m1_parkerb) /
		sizeof(kal_uint16));
}
...
static void capture_setting(void)
{
	LOG_INF("E\n");
	gc02m1_table_write_cmos_sensor(
		addr_data_pair_capture_gc02m1_parkerb,
		sizeof(addr_data_pair_capture_gc02m1_parkerb) /
		sizeof(kal_uint16));
}
...
static void normal_video_setting(void)
{
	LOG_INF("E\n");
	gc02m1_table_write_cmos_sensor(
		addr_data_pair_normal_video_gc02m1_parkerb,
		sizeof(addr_data_pair_normal_video_gc02m1_parkerb) /
		sizeof(kal_uint16));
}
...
static void hs_video_setting(void)
{
	LOG_INF("E\n");
	gc02m1_table_write_cmos_sensor(
		addr_data_pair_hs_video_gc02m1_parkerb,
		sizeof(addr_data_pair_hs_video_gc02m1_parkerb) /
		sizeof(kal_uint16));
}
...
static void slim_video_setting(void)
{
	LOG_INF("E\n");
	gc02m1_table_write_cmos_sensor(
		addr_data_pair_slim_video_gc02m1_parkerb,
		sizeof(addr_data_pair_slim_video_gc02m1_parkerb) /
		sizeof(kal_uint16));
}
...
static void read_parkerb_hlt_micro_gc02m1_SNData(UINT8 *parkerb_hlt_micro_gc02m1_SNData,
    ...
    for(i = 0; i < 17; i++){
		parkerb_hlt_micro_gc02m1_SNData[i] = (kal_int8)read_cmos_eeprom_8(start_addr + i);
		LOG_INF("read_cmos_eeprom_8(start_addr + i) = 0x%x\n", read_cmos_eeprom_8(start_addr + i));
		checksum = checksum + parkerb_hlt_micro_gc02m1_SNData[i];
	}
	checksum = checksum % 255;
	if(read_cmos_eeprom_8(start_addr+17) == 1 && read_cmos_eeprom_8(start_addr+18) == checksum ) {
		*feature_para_len = i;
		LOG_INF("parkerb_hlt_micro_gc02m1_SNData checksum sucess, feature_para_len；%d", *feature_para_len);
	} else {
		*feature_para_len = 0;
		LOG_INF("parkerb_hlt_micro_gc02m1_SNData checksum failed");
	}
}
#endif
...
static kal_uint16 read_module_id(kal_uint32 addr)
{
    ...
    char pusendcmd[2] = {(char)(addr >> 8) , (char)(addr & 0xFF) };
    iReadRegI2C(pusendcmd , 2, (u8*)&get_byte,1,0xA4);
    LOG_INF("HLT gc02m1 module_id: 0x%x\n", get_byte);
    return get_byte;
}
...
#ifdef ENABLE_GC02M1_OTP
				if(read_module_id(0x0000) == 0x09){
					*sensor_id = imgsensor_info.sensor_id;
					LOG_INF("HLT gc02m1 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
					if(deviceInfo_register_value == 0x00){
						register_imgsensor_deviceinfo("Cam_r2", DEVICE_VERSION_GC02M1, read_module_id(0x0000));
					...
					LOG_INF("HLT Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				}
				LOG_INF("sensor_id 0x%x,imgsensor_info.sensor_id 0x%x",*sensor_id,imgsensor_info.sensor_id);
#endif
			}
			LOG_INF("HLT read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_SERIANO_IC:
		read_parkerb_hlt_micro_gc02m1_SNData(feature_data_8, feature_para_len);
