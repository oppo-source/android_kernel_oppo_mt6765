...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = YOGURT_CXT_MICRO_GC02K0_SENSOR_ID,
	.checksum_value = 0xf7375923,

	.pre = {
		.pclk = 37000000,/*darfur change 0730 24 592M*/
		.linelength = 1079,
		.framelength = 1248,/*darfur change 0730 24 592M*/
		.startx = 0,
		.starty = 0,
		...
		do {
			*sensor_id = return_sensor_id() & 0xffef;
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if(read_module_id(0x0000) == 0x1d){
					*sensor_id = imgsensor_info.sensor_id;
					//hardwareinfo_set_prop(HARDWARE_MONO_CAM_MOUDULE_ID, "Cxt");
					pr_info("CXT i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
					return ERROR_NONE;
				}
				else{
					pr_info("CXT Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
...
UINT32 GC02K0_CXT_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
