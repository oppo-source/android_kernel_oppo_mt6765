...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = YOGURT_SHENGTAI_MICRO_GC02K0_SENSOR_ID,
...
static kal_uint16 read_module_id(kal_uint32 addr)
{
    ...
    LOG_INF("Shengtai module_id: 0x%x\n", get_byte);
    return get_byte;
}
		...
		do {
			*sensor_id = return_sensor_id() & 0xffef;
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if(read_module_id(0x0000) == 0x00 || read_module_id(0x0000) == 0x07){
					*sensor_id = imgsensor_info.sensor_id;
					//hardwareinfo_set_prop(HARDWARE_MONO_CAM_MOUDULE_ID, "Shengtai");
					pr_info("Shengtai i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
					return ERROR_NONE;
				}
				else{
...
UINT32 GC02K0_SHENGTAI_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
