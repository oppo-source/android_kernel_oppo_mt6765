...
#include "parkerb_shinetech_ov02b1bmipiraw_Sensor.h"
...
#define DEVICE_MODULE_ID_OV02B1B 0x07
...
static  struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = PARKERB_SHINETECH_MONO_OV02B1B_SENSOR_ID,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			write_cmos_sensor(0xfd, 0x00);
			*sensor_id = ((read_cmos_sensor(0x02) << 8) | read_cmos_sensor(0x03) + 6);
...
//				hardwareinfo_set_prop(HARDWARE_BACK_SUB_CAM_MOUDULE_ID, "ShengTai");
				if(deviceInfo_register_value == 0x00){
					register_imgsensor_deviceinfo("Cam_r1", DEVICE_VERSION_OV02B1B, DEVICE_MODULE_ID_OV02B1B);
