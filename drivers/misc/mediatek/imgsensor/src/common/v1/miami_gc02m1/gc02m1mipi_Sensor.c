...
#define PFX "miami_gc02m1_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = MIAMI_GC02M1_SENSOR_ID,
...
/* 20200509, modify for dual camera framesync*/
				     ...
				     kal_uint16 target_frame_length)
{
	...
	if (imgsensor.autoflicker_en) {
		if(realtime_fps >= 297 && realtime_fps <= 305)
			...
			set_dummy();/* 20200523, modify for dual camera framesync*/
	} else
		set_dummy();/* 20200523, modify for dual camera framesync*/
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("GC02M1(Y1&Y6) i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				read_EepromSn();
				return ERROR_NONE;
			}
