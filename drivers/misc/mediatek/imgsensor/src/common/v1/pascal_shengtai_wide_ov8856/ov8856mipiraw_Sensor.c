	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			printk("ShengTai wenhui i2c write id: *sensor_id = 0x%x,imgsensor_info.sensor_id =%x\n",*sensor_id,imgsensor_info.sensor_id);
			if (*sensor_id == OV8856_SENSOR_ID) {
				module_id = read_module_id();
				if (module_id == 0x07) {
					*sensor_id = imgsensor_info.sensor_id;
//					hardwareinfo_set_prop(HARDWARE_WIDE_ANGLE_CAM_MOUDULE_ID, "ShengTai");
					LOG_INF("ShengTai i2c write id: 0x%x, sensor id: 0x%x, ver = 0x%x<0xB1=r2a,0xB0=r1a>\n",
						imgsensor.i2c_write_id, *sensor_id, read_cmos_sensor(0x302A));
					return ERROR_NONE;
				}else {
					*sensor_id = 0xFFFFFFFF;
					pr_err("ShengTai wide camera match fail");
...
UINT32 OV8856_SHENGTAI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
