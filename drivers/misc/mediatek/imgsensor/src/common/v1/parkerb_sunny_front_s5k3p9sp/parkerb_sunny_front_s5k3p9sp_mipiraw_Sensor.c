...
#include "parkerb_sunny_front_s5k3p9sp_mipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
		.sensor_id = PARKERB_SUNNY_FRONT_S5K3P9SP_SENSOR_ID,
		#ifdef OPLUS_FEATURE_CAMERA_COMMON
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[8] =
{
	...
	{ 4640,  3488,    0,    448, 4640, 2592, 2320,  1296, 0000, 0000, 2320,  1296,      0,    0, 2320, 1296}, /* slim video*/
	...
	{ 4640,  3488,    0,    0, 4640, 3488, 4640,  3488, 0000, 0000, 4640,  3488,      0,    0, 4640, 3488}, //custom3 capture remosic
}; /*cpy from preview*/
...
unsigned int parkerb_read_4cell_data(char *data)
{
	if (data != NULL) {
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
static void read_parkerb_sunny_front_s5k3p9sp_SNData(UINT8 *parkerb_sunny_front_s5k3p9sp_SNData,
    ...
    for(i = 0; i < 17; i++){
		parkerb_sunny_front_s5k3p9sp_SNData[i] = (kal_int8)read_cmos_eeprom_8(start_addr + i);
		LOG_INF("read_cmos_eeprom_8(start_addr + i) = 0x%x\n", read_cmos_eeprom_8(start_addr + i));
		checksum = checksum + parkerb_sunny_front_s5k3p9sp_SNData[i];
	}
	checksum = checksum % 255;
	if(read_cmos_eeprom_8(start_addr+17) == 1 && read_cmos_eeprom_8(start_addr+18) == checksum ) {
		*feature_para_len = i;
		LOG_INF("parkerb_sunny_front_s5k3p9sp_SNData checksum sucess, feature_para_len；%d", *feature_para_len);
	} else {
		*feature_para_len = 0;
		LOG_INF("parkerb_sunny_front_s5k3p9sp_SNData checksum failed");
	}
}
#endif
...
#ifdef USE_TNP_BURST
const u16 parkerb_uTnpArrayInit[] = {
	0x126F,
	0x0000,
	0x0000,
	0x4906,
...
static void burst_read_to_check(void)
{
	...
	for(i = 0; i < (u16)sizeof(parkerb_uTnpArrayInit); i++)
	{
...
static void sensor_init(void)
{
	...
	iWriteRegI2CTiming((u8*)parkerb_uTnpArrayInit, (u16)sizeof(parkerb_uTnpArrayInit), imgsensor.i2c_write_id, imgsensor_info.i2c_speed);
	write_cmos_sensor_16_16(0x6028, 0x4000);
	write_cmos_sensor_16_16(0x6004, 0x0000); //TNP burst end

	table_write_cmos_sensor(addr_data_pair_global,sizeof(addr_data_pair_global) / sizeof(kal_uint16));
	#else
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	*/
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = (((read_cmos_sensor_16_8(0x0000) << 8) | read_cmos_sensor_16_8(0x0001))+2);
    ...
    LOG_INF("[%s]s5k3p9sp sensor id: %d", __func__, (read_cmos_sensor_16_8(0x0000) << 8) | read_cmos_sensor_16_8(0x0001) + 1);
	if (*sensor_id !=  imgsensor_info.sensor_id) {
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	*feature_para_len = 4;
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:
		{
			...
			if (type == FOUR_CELL_CAL_TYPE_ALL) {
				...
				parkerb_read_4cell_data((char *)(*(feature_data+1)));
	...
	case SENSOR_FEATURE_GET_SERIANO_IC:
		read_parkerb_sunny_front_s5k3p9sp_SNData(feature_data_8, feature_para_len);
