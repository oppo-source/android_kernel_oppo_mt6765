...
#if defined(CONFIG_MTK_20271) || defined(CONFIG_MTK_21281) || defined(CONFIG_MTK_21285)
...
static struct pinctrl * pinctrl_otg = NULL;
static struct pinctrl_state * pinctrl_iddig = NULL;
...
static void _set_vbus(int is_on)
{
	...
	if (is_on && !vbus_on) {
		...
		charger_dev_set_boost_current_limit(primary_charger, 1400000);
#else
