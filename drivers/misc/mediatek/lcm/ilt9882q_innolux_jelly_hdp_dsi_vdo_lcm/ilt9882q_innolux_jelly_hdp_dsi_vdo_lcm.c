...
** ODM_WT_EDIT
** File: - ili9882q_innolux_hdp_dsi_vdo_lcm.c
** Description: source file for lcm ili9882q+innolux in kernel stage
**
** Version: 1.0
** Date : 2019/9/25
**
...
**  lianghao       2019/9/25     1.0     source file for lcm ili9882q+innolux in kernel stage
**
****************************************************************/
...
static void lcm_get_params(struct LCM_PARAMS *params)
{
...
#if (LCM_DSI_CMD_MODE)
	params->dsi.PLL_CLOCK = 367;	/* this value must be in MTK suggested table */
#else
	params->dsi.data_rate = 737;
	...
	/* add /proc/devinfo/lcd node*/
	register_device_proc("lcd", "ili9882q", "innolux_ilitek");
}
...
static void lcm_init(void)
{
	...
	if (lcm_dsi_mode == CMD_MODE) {
		...
		pr_debug("ili9882q_innolux_lcm_mode = cmd mode :%d----\n", lcm_dsi_mode);
	} else {
		push_table(NULL, init_setting_vdo, sizeof(init_setting_vdo) / sizeof(struct LCM_setting_table), 1);
		pr_debug("ili9882q_innolux_lcm_mode = vdo mode :%d\n", lcm_dsi_mode);
	}
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	printk("[HW check backlight ili9882q+innolux]level = %d, para_list[0] = %x,para_list[1] = %x\n",
...
static unsigned int lcm_esd_recover(void)
{
	...
	if (lcm_dsi_mode == CMD_MODE) {
		push_table(NULL, init_setting_cmd, sizeof(init_setting_cmd) / sizeof(struct LCM_setting_table), 1);
		pr_debug("ili9882q_innolux_lcm_mode = cmd mode esd recovery :%d----\n", lcm_dsi_mode);
	} else {
		push_table(NULL, init_setting_vdo, sizeof(init_setting_vdo) / sizeof(struct LCM_setting_table), 1);
		pr_debug("ili9882q_innolux_lcm_mode = vdo mode esd recovery :%d----\n", lcm_dsi_mode);
	}
	lcm_set_cabc_cmdq(NULL, cabc_status);
...
struct LCM_DRIVER ilt9882q_innolux_jelly_hdp_dsi_vdo_lcm_drv = {
	.name = "ilt9882q_innolux_jelly_hdp_dsi_vdo_lcm",
