...
static void lcm_get_params(struct LCM_PARAMS *params)
{
	...
	register_device_proc("lcd", "ili9882n", "boe_soda_ilitek");

}

static void lcm_init_power(void)
{
	pr_debug("boe_soda9882n lcm_init_power\n");
	MDELAY(1);
	SET_LCM_VSP_PIN(1);
	MDELAY(3);
	SET_LCM_VSN_PIN(1);
...
{
	pr_debug("boe_soda9882n lcm_suspend_power\n");
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	pr_err("[HW check backlight boe_soda9882n]level=%d,para_list[0]=%x,para_list[1]=%x\n",level,bl_level[0].para_list[0],bl_level[0].para_list[1]);
...
static unsigned int lcm_esd_recover(void)
{
	...
	if (lcm_dsi_mode == CMD_MODE) {
		push_table(NULL, init_setting_cmd, sizeof(init_setting_cmd) / sizeof(struct LCM_setting_table), 1);
		pr_debug("boe_soda9882n = cmd mode esd recovery :%d----\n", lcm_dsi_mode);
	} else {
		push_table(NULL, init_setting_vdo, sizeof(init_setting_vdo) / sizeof(struct LCM_setting_table), 1);
		pr_debug("boe_soda9882n = vdo mode esd recovery :%d----\n", lcm_dsi_mode);
	}
...
static struct LCM_setting_table set_gamma_exit[] = {
	{0xFF, 0x03, {0x98, 0x82, 0x08}},
	{0xE0, 0x1B, {0x00, 0x24, 0x4E, 0x71, 0xA3, 0x50, 0xD1, 0xF7, 0x26, 0x4E, 0x95, 0x8F, \
				0xC6, 0xF7, 0x28, 0xAA, 0x59, 0x94, 0xB8, 0xE4, 0xFF, 0x0A, 0x38, 0x6F, 0x9D, 0x03, 0xEC}},
	{0xE1, 0x1B, {0x00, 0x24, 0x4E, 0x71, 0xA3, 0x50, 0xD1, 0xF7, 0x26, 0x4E, 0x95, 0x8F, \
				0xC6, 0xF7, 0x28, 0xAA, 0x59, 0x94, 0xB8, 0xE4, 0xFF, 0x0A, 0x38, 0x6F, 0x9D, 0x03, 0xEC}},
	{0xFF, 0x03, {0x98, 0x82, 0x00}},
};
...
static void lcm_set_gamma_mode_cmdq(void *handle, unsigned int level)
{
	...
	pr_err("[HW check backlight soda9882n_huaxian]current_level= %d, backlight_level=%d,para_list[0]=%x,para_list[1]=%x\n",
...
struct LCM_DRIVER oplus20361_boe_soda9882n_hdp_dsi_vdo_lcm_drv = {
	.name = "oplus20361_boe_soda9882n_hdp_dsi_vdo_lcm",
