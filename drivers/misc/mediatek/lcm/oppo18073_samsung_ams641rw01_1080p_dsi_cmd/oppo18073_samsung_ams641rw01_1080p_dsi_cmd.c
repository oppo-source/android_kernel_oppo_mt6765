...
static void push_table22(void *handle, struct LCM_setting_table *table,
...
static void lcm_aod(int enter)
{
	if (enter == 1) {
		...
		push_table(lcm_aod_in_setting,
			sizeof(lcm_aod_in_setting) / sizeof(struct LCM_setting_table), 1);
	}
...
{
	push_table22(handle, lcm_aod_display_on_setting,
		sizeof(lcm_aod_display_on_setting) / sizeof(struct LCM_setting_table), 1);
...
{
	if (hbm_en) {
		push_table22(handle, lcm_aod_out_from_hbm_setting,
			sizeof(lcm_aod_out_from_hbm_setting) / sizeof(struct LCM_setting_table), 1);
		LCD_DEBUG("[soso] aod out from hbm\n");

	} else {
		push_table22(handle, lcm_aod_out_setting,
			sizeof(lcm_aod_out_setting) / sizeof(struct LCM_setting_table), 1);
...
{
	MDELAY(5);
	push_table(lcm_initialization_cmd_setting,
		sizeof(lcm_initialization_cmd_setting) / sizeof(struct LCM_setting_table), 1);
	hbm_en = false;
...
static void lcm_suspend(void)
{
	push_table(lcm_sleep_in_setting,
		sizeof(lcm_sleep_in_setting) / sizeof(struct LCM_setting_table), 1);
	hbm_en = false;
...
static int oppo_lcm_dc_backlight(void *handle, unsigned int level, int hbm_en)
{
	...
	for (i = 0; i < sizeof(lcm_seed_setting) / sizeof(lcm_seed_setting[0]); i++) {
		...
		for (k = 0; k < seed_table[i].count; k++) {
			seed_table[i].para_list[k] = seed_table[i].para_list[k] *
				(255 - seed_alpha) / 255;
		}
	}
	...
	if (aod_in && !aod_state) {
		...
		if (!hbm_en) {
			...
			push_table22(handle, lcm_backlight_level_setting,
				sizeof(lcm_backlight_level_setting) / sizeof(lcm_backlight_level_setting[0]),
				1);
		}
	}
}
...
static bool lcm_set_hbm_cmdq(bool en, void *qhandle)
{
	...
	if (en) {
		...
		if (!aod_state) {
			...
			if (qhandle == NULL) {
				push_table(lcm_aod_from_display_on,
					sizeof(lcm_aod_from_display_on) / sizeof(struct LCM_setting_table), 1);
				printk("[soso] %s : %d ! wfp back aod from doze_suspend!\n", __func__, en);

			} else {
				push_table22(qhandle, lcm_aod_from_display_on,
					sizeof(lcm_aod_from_display_on) / sizeof(struct LCM_setting_table), 1);
