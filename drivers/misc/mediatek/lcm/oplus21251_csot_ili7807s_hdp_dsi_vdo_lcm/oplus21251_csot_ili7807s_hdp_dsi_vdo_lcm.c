...
static struct LCM_setting_table init_setting_vdo[] = {
	...
	{0xFF, 0x03, {0x78, 0x07, 0x00}},
	{0x53, 0x01, {0x24}},
	{0x55, 0x01, {0x02}},			/*cabc 11.5 percent*/
	{0x35, 0x01, {0x00}},
	{0x11, 0x01, {0x00}},
	{REGFLAG_DELAY, 100, {}},

	 {0xFF, 0x03, {0x78, 0x07, 0x08}},
        {0xE0, 0x1F, {0x00, 0x00, 0x36, 0x74, 0x10, 0xBD, 0xED, 0x11, 0x15, 0x48, 0x72, \
        0xAF, 0x29, 0xDA, 0x1F, 0x54, 0x2A, 0x87, 0xC0, 0xE6, 0x3F, 0x15, 0x39, 0x5B, 0x3F, 0x76, 0x96, 0xC4, 0x0F, 0xD5, 0xD7}},
        {0xE1, 0x1F, {0x00, 0x00, 0x36, 0x74, 0x10, 0xBD, 0xED, 0x11, 0x15, 0x48, 0x72, \
        0xAF, 0x29, 0xDA, 0x1F, 0x54, 0x2A, 0x87, 0xC0, 0xE6, 0x3F, 0x15, 0x39, 0x5B, 0x3F, 0x76, 0x96, 0xC4, 0x0F, 0xD5, 0xD7}},
};
...
static void lcm_get_params(struct LCM_PARAMS *params)
{
	...
	register_device_proc("lcd", "csot7807s", "21251_boe_csottek");
}

static void lcm_init_power(void)
{
	pr_debug("csot_csot7807s lcm_init_power\n");
	MDELAY(1);
	SET_LCM_VSP_PIN(1);
	MDELAY(3);
	SET_LCM_VSN_PIN(1);
...
{
	pr_debug("csot_csot7807s lcm_suspend_power\n");
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	pr_err("[HW check backlight csot_csot7807s]level = %d, para_list[0] = %x, para_list[1] = %x\n",
...
/* 2020/9/1, modify lcd cabc to hs download*/
...
static unsigned int lcm_esd_recover(void)
{
	...
	if (lcm_dsi_mode == CMD_MODE) {
		push_table(NULL, init_setting_cmd, sizeof(init_setting_cmd) / sizeof(struct LCM_setting_table), 1);
		pr_debug("csot_csot7807s = cmd mode esd recovery :%d----\n", lcm_dsi_mode);
	} else {
		push_table(NULL, init_setting_vdo, sizeof(init_setting_vdo) / sizeof(struct LCM_setting_table), 1);
                push_table(NULL, lcm_display_on, sizeof(lcm_display_on) / sizeof(struct LCM_setting_table), 1);
		pr_debug("csot_csot7807s = vdo mode esd recovery :%d----\n", lcm_dsi_mode);
	}
...
static struct LCM_setting_table_V3 set_gamma_enter[] = {
        ...
        {0x29, 0xE0, 0x1F, {0x15, 0x93, 0x96, 0x9C, 0x15, 0xA7, 0xB2, 0xBC, 0x15, 0xCF, 0xE1, \
        0xFF, 0x2A, 0x19, 0x47, 0x73, 0x2A, 0x9F, 0xD5, 0xF9, 0x3F, 0x26, 0x48, 0x69, 0x3F, 0x81, 0x9F, 0xC5, 0x0F, 0xD6, 0xD7}},
        {0x29, 0xE1, 0x1F, {0x15, 0x93, 0x96, 0x9C, 0x15, 0xA7, 0xB2, 0xBC, 0x15, 0xCF, 0xE1, \
        0xFF, 0x2A, 0x19, 0x47, 0x73, 0x2A, 0x9F, 0xD5, 0xF9, 0x3F, 0x26, 0x48, 0x69, 0x3F, 0x81, 0x9F, 0xC5, 0x0F, 0xD6, 0xD7}},
...
static struct LCM_setting_table_V3 set_gamma_exit[] = {
        {0x29, 0xFF, 0x03, {0x78, 0x07, 0x08}},
        {0x29, 0xE0, 0x1F, {0x00, 0x00, 0x36, 0x74, 0x10, 0xBD, 0xED, 0x11, 0x15, 0x48, 0x72, \
        0xAF, 0x29, 0xDA, 0x1F, 0x54, 0x2A, 0x87, 0xC0, 0xE6, 0x3F, 0x15, 0x39, 0x5B, 0x3F, 0x76, 0x96, 0xC4, 0x0F, 0xD5, 0xD7}},
        {0x29, 0xE1, 0x1F, {0x00, 0x00, 0x36, 0x74, 0x10, 0xBD, 0xED, 0x11, 0x15, 0x48, 0x72, \
        0xAF, 0x29, 0xDA, 0x1F, 0x54, 0x2A, 0x87, 0xC0, 0xE6, 0x3F, 0x15, 0x39, 0x5B, 0x3F, 0x76, 0x96, 0xC4, 0x0F, 0xD5, 0xD7}},
        {0x29, 0xFF, 0x03, {0x78, 0x07, 0x00}},
};
...
static void lcm_set_gamma_mode_cmdq(void *handle, unsigned int level)
{
	...
	/*
	   ...
	   pr_err("[HW check backlight csot_csot7807s]current_level = %d, backlight_level = %d, para_list[0]=%x, para_list[1] = %x\n",
...
struct LCM_DRIVER oplus21251_csot_ili7807s_hdp_dsi_vdo_lcm_drv = {
	.name = "oplus21251_csot_ili7807s_hdp_dsi_vdo_lcm",
