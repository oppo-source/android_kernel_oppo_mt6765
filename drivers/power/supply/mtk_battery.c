...
#define BAT_TYPE__LIWINON_4450mV_NTC_MIN 7
...
try_again:
	...
	if ((battery_type == BAT_TYPE__ATL_4450mV) || (battery_type == BAT_TYPE__COSMX_4450mV) || battery_type == BAT_TYPE__LIWINON_4450mV) {
		return true;
	} else {
		if (retry_flag == 0) {
			retry_flag = 1;
...
bool meter_fg_30_get_battery_authenticate(void)
{
	...
	if((battery_id == 2) || (battery_id == BAT_COSMX_BATT_ID) ||(battery_id == BAT_LIWINON_BATT_ID))
