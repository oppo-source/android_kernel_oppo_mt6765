...
enum oppo_batt_num_445v_5k{
	OPPO_LIWINON_445V_5K = 0,
	OPPO_COSLIGHT_445V_5K,
	OPPO_ATL_445V_5K,
	OPPO_MAX_NUM_445V_5K
};
...
oppo_batt_data oppo_battery_id_vol[OPPO_MAX_NUM] = {[0]={790000,1040000},[1]={200000,360000},[2]={70000,180000}};
oppo_batt_data oppo_battery_id_445v_5k[OPPO_MAX_NUM_445V_5K] = {[0]={70000,180000},[1]={180000,350000},[2]={550000,790000}};
#endif
...
enum {
	...
	BAT_TYPE__COSMX_4450mV, //286mV~299mV
...
int battery_type_check(int *battery_type)
{
	...
	oplus_gpio_value_switch(BOARD_TEMP_GPIO_NUM, 0);
	msleep(300);
#else
...
#if defined(CONFIG_MTK_20271) || defined(CONFIG_MTK_20361)
	msleep(10);
	oplus_gpio_value_switch(BOARD_TEMP_GPIO_NUM, 1);
	...
	value = ret_value;
#endif
	printk(KERN_ERR "[battery_type_check]: batt_id_volt[%d]\n",value );

	if(is_fuelgauge_apply() == true){
#if defined(CONFIG_MTK_20271) || defined(CONFIG_MTK_20361)
		value = value * 1000;
		battery_id = -1;
#ifdef CONFIG_MTK_20361
		offset = 1;
		for (id = 0; id < OPPO_MAX_NUM_445V_5K; id++) {
			if (value > oppo_battery_id_445v_5k[id].id_vol_l && value < oppo_battery_id_445v_5k[id].id_vol_h) {
				battery_id = id;
				break;
			}
		}
		...
		for (id = 0; id < OPPO_MAX_NUM; id++) {
				if (value > oppo_battery_id_vol[id].id_vol_l && value < oppo_battery_id_vol[id].id_vol_h) {
					battery_id = id;
					break;
				}
			}
		...
		switch(battery_id){
			...
			case 1:
				*battery_type = BAT_TYPE__SDI_4400mV;
				break;
			case 2:
				*battery_type = BAT_TYPE__LIWION_4400mV;
				break;
			default:
				*battery_type = BAT_TYPE__UNKNOWN;
				break;
...
/*add for battery data prase*/
		if((battery_id >= 0)&&(battery_id < 3)) {
			battery_id = battery_id + offset;
		} else {
			battery_id = 0;
		}
#else
		if (!(is_project(20375) || is_project(20376)
			...
			|| is_project(21251) || is_project(21253)
			|| is_project(21254) )) {
	...
	if(is_project(21281) || is_project(21282) || is_project(21283) || is_project(21285)){
		if (value >= 240 && value <= 350) {
			*battery_type = BAT_TYPE__SDI_4400mV;
			battery_id = 1;
		} else if (value >= 820 && value <= 1100) {
			*battery_type = BAT_TYPE__ATL_4400mV;
			battery_id = 0;
		}else{
			*battery_type = BAT_TYPE__UNKNOWN;
...
static void register_battery_devinfo(void)
{
	...
	switch (battery_type) {
		...
		case BAT_TYPE__COSMX_4450mV:
			...
			manufacture = "COSMX";
			break;
		case BAT_TYPE__NVT_4450mV:
			version = "4.45v";
			manufacture = "NVT";
			break;
		case BAT_TYPE__LIWION_4400mV:
			version = "4.40v";
			manufacture = "LIWINON";
			break;
#endif
	...
	struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	union power_supply_propval bat_pval = {0, };
	...
	case ADC_CHANNEL_READ:
		...
		if (adc_in_data[0] == 0) { /* I_SENSE */
			power_supply_get_property(bat_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &bat_pval);
			adc_out_data[0] = bat_pval.intval * adc_in_data[1];
		} else if (adc_in_data[0] == 1) { /* BAT_SENSE */
			power_supply_get_property(bat_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &bat_pval);
			adc_out_data[0] = bat_pval.intval * adc_in_data[1];
		} else if (adc_in_data[0] == 3) { /* V_Charger */
			power_supply_get_property(bat_psy, POWER_SUPPLY_PROP_CHARGE_NOW, &bat_pval);
			adc_out_data[0] = bat_pval.intval * adc_in_data[1];
			/* adc_out_data[0] = adc_out_data[0] / 100; */
		} else if (adc_in_data[0] == 30) {
			/* V_Bat_temp magic number */
			power_supply_get_property(bat_psy, POWER_SUPPLY_PROP_TEMP, &bat_pval);
			adc_out_data[0] = bat_pval.intval * adc_in_data[1] / 10;
		} else if (adc_in_data[0] == 66) {
			power_supply_get_property(bat_psy, POWER_SUPPLY_PROP_CURRENT_NOW, &bat_pval);
			adc_out_data[0] = bat_pval.intval / 10;
		} else {
		...
		bm_err("*** unlocked_ioctl : Channel %d * %d times = %d\n",
			    adc_in_data[0], adc_in_data[1], adc_out_data[0]);
		break;
	default:
		bm_err("**** unlocked_ioctl unknown IOCTL: 0x%08x %d\n", cmd, __LINE__);
		break;
	}
